...
const SideDrawer = ({ isDrawerOpen, handleCloseDrawer }) => {
  ...
  return (
    <>
      ...
      {isLoggedIn && (
          <aside
            className={`fixed max-w-screen-xl top-[75px] rounded-b-lg right-0 z-20 flex w-[300px] flex-col gap-4 bg-white p-4 shadow-sm ${isDrawerOpen ? "translate-y-0 opacity-100" : "-translate-y-full opacity-0"} transition-all duration-300 ease-in-out max-md:hidden`}
            style={{ right: "max(calc(50% - 650px), 0rem)" }}
          >
      ...
      >
        {isLoggedIn && <ProfileSegment handleCloseDrawer={handleCloseDrawer} />}
